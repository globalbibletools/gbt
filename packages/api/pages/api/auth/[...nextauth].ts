...
export const authOptions: AuthOptions = {
  ...
  callbacks: {
    // This callback allows us to get other information about the user for use in access control.
    async session({ session, user }) {
      if (session.user) {
        session.user.id = user.id;

        const roles = await client.userSystemRole.findMany({
          where: {
            userId: user.id,
          },
        });
        session.user.systemRoles = roles.map(({ role }) => role);
      }
