/**
 * This is from https://github.com/arthurtyukayev/use-keyboard-shortcut
 */
...
const useKeyboardShortcut = (
  ...
  const keydownListener = useCallback(
    (keydownEvent: KeyboardEvent) => {
      ...
      const element = keydownEvent.target
        ? (keydownEvent.target as HTMLElement)
        : null;
      if (
        options.ignoreInputFields &&
        BLACKLISTED_DOM_TARGETS.indexOf(element?.tagName ?? '') >= 0
      ) {
        return;
      }
